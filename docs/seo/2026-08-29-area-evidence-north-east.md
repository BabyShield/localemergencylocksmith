...
- Inventory: 21 canonical slugs from the current `Nuneaton` and `Rugby` display clusters in `AREAS`.
- Administrative split: 12 slugs are in Nuneaton and Bedworth Borough, `hartshill` is in North Warwickshire Borough, and the eight Rugby-cluster slugs are in Rugby Borough.
- Facts below were derived afresh from opened primary sources. Existing area-page assertions were not treated as evidence.
- A context tag records only what the cited source directly establishes. It is not permission to generalise from a mapped core, ward, named asset or historical study to every address represented by a route slug.
- A conditional service-use implication is a writing constraint. It does not establish service coverage, response time, access conditions, property type, lock type, demand or job history.

## Nuneaton display cluster
