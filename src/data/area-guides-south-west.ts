...
const SERVICE_CHECKS: Record<ServiceAreaSlug, [string, string, string]> = {
  'emergency-lockout': [
    'Verify identity, authority and the controlled entrance.',
    'Inspect the complete door set before deciding scope.',
    'Explain the expected cost and approve any variation.',
  ],
  'lock-change': [
    'Confirm who may authorise the specific replacement.',
    'Inspect door, frame, hinges and existing lock together.',
    'Approve specification, property permissions and price first.',
  ],
  'upvc-lock-repair': [
    'Record handle, key and open-or-closed door behaviour.',
    'Identify the installed mechanism from direct evidence.',
    'Agree the supported diagnosis, scope and cost.',
  ],
  'boarding-up': [
    'Name each damaged opening and authorised contact.',
    'Preserve possible evidence until police directions allow work.',
    'Agree the temporary external scope and price.',
  ],
  'lock-upgrade': [
    'Document the whole existing door assembly.',
    'Obtain any requirement and property permission in writing.',
    'Compare compatible accredited options and agree cost.',
  ],
}
...
function buildServiceGuidance(
  ...
  return {
    heading: `${SERVICE_LABELS[service]} for ${areaName}`,
    body: [`${context.local} ${firstProtocol}`, `${context.decision} ${secondProtocol}`],
    checks: [
      `Use the complete ${areaName} address and identify the exact affected opening.`,
      ...SERVICE_CHECKS[service],
    ],
    faq: {
      q: `What should I provide for ${SERVICE_LABELS[service]} at an address in ${areaName}?`,
      a: `${context.local} ${SERVICE_CHECKS[service].join(' ')} ${context.decision}`,
    },
  }
}
...
function buildGuide(seed: AreaGuideSeed, areaIndex: number): GovernedAreaGuide {
  const serviceGuidance = Object.fromEntries(
    SERVICE_AREA_SLUGS.map((service, serviceIndex) => [
      service,
      buildServiceGuidance(seed.name, service, seed.contexts[service], areaIndex + serviceIndex * 11),
    ]),
    ...
    faqs: [
      {
        ...
        a: `No. They provide locality-scale planning, conservation or transport context only. The exact opening, installed hardware, condition, authority and any property-specific controls must be checked separately.`,
      },
      {
