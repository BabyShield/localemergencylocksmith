// Pillar 4: uPVC Door Locks — Full blog post content
// 6 NEW posts (excludes upvc-door-lock-needs-replacing which already exists)

export const PILLAR_4_CONTENT: Record<string, { body: string; faqs: { q: string; a: string }[] }> = {
...
1. **You close the door** — the latch (a spring-loaded bolt on the gearbox) clicks into the frame, holding the door shut but not locked.
2. **You lift the handle** — this activates the gearbox, which pushes the hooks out at the top and bottom, and extends the rollers into their frame keeps. The door is now compressed tightly against the seals.
3. **You turn the key** — this rotates the euro cylinder, which in turn throws the deadbolt within the gearbox. The door is now fully locked at all points.
...
1. **PZ distance** — this is the distance from the centre of the handle spindle hole to the centre of the keyhole (euro cylinder). Common PZ distances are 70mm and 92mm. This is the most critical measurement.
2. **Backset** — the distance from the front edge of the mechanism (the faceplate) to the centre of the keyhole. Common backsets are 35mm and 45mm.
3. **Overall length** — the total length of the mechanism from top to bottom. This varies significantly between brands and door sizes.
...
- The handle feels loose or does not spring back after lifting
- You have to lift the door by the handle to get it to lock
- The key is increasingly stiff to turn
...
- A tape measure or ruler (millimetre accuracy)
- Your door key
- A pen and paper or your phone to note measurements
...
`,
    faqs: [
      ...
      },
      {
        ...
        a: 'You can get a rough measurement by measuring the door thickness and subtracting a few millimetres for the handle and escutcheon on each side, but this is not reliable. The only accurate way is to remove the cylinder by undoing the fixing screw on the door edge, turning the key slightly to align the cam, and pulling the cylinder out. Then measure from the screw hole to each end. It takes five minutes and avoids buying the wrong size.',
      },
      {
  ...
  'best-euro-cylinder-upgrades-2026': {
    body: `
...
Spring is recovery time. Your door has been through a Coventry winter — rain, frost, cold winds — and it needs attention.
...
Summer brings its own challenges — mainly heat expansion.
...
Give the uPVC panels a wash with warm soapy water. Do not use abrasive cleaners or solvents — they can damage the uPVC surface. While you are cleaning, check for any cracks in the panels or glazing beads. Damaged glazing beads are a security risk as they can allow glass to be removed from outside.
...
Inspect the rubber seals around the door frame. Press on them — they should be springy and resilient. If they are flat, cracked, or hard, they will not keep out rain and draughts over winter. Replace any damaged seals now, before the cold weather arrives.
...
3. **Seized locking points** — the hooks or rollers have seized in position due to corrosion or lack of lubrication. The handle cannot push them out because they are stuck.
...
1. **Press the handle down.** Does it spring back to horizontal? If no, it is likely a broken cassette spring (Problem 1).
2. **Wiggle the handle and backplate.** Does it move? If yes, it is loose screws or a worn spindle (Problem 2).
3. **Try to lift the handle.** Does it jam or refuse to lift? If yes, it is likely misalignment or a mechanism issue (Problem 3).
4. **Is the handle physically broken?** If yes, you need a replacement set (Problem 4).
...
**DIY is fine for:**
- Tightening loose handle screws
- Replacing a handle set (if you are comfortable with basic tools and can match the PZ distance)
- Replacing a worn spindle

**Call a locksmith for:**
- Handle that will not lift to lock (requires diagnosis and potentially mechanism replacement)
- Repeated handle failures (something else is causing the problem)
