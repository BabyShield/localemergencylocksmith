export const PILLAR_1_CONTENT: Record<
  string,
  { body: string; faqs: { q: string; a: string }[] }
> = {
...
1. The burglar grips the exposed part of the cylinder with pliers or a dedicated snapping tool
2. They apply force to one side, bending the cylinder until it fractures at its weakest point — the screw hole in the middle
...
If you are unsure what is fitted, a [lock upgrade assessment](/services/lock-upgrade) can identify the cylinder, door furniture and relevant certification before any replacement is recommended. Check the [prices page](/prices) for current rates or call 024 7522 4730 for a quote.`,
  ...
  'euro-cylinder-locks-explained': {
    body: `## What Is a Euro Cylinder Lock?
...
A single euro cylinder has a keyhole on one side only. The other side is blank. These are typically used on doors where you don't need key access from both sides — garage doors, some back doors, and certain commercial applications. They're less common on front doors.
...
Price or appearance alone does not establish a cylinder's performance. Check the exact independent certification, correct size and complete door-and-frame condition. A certified cylinder can improve resistance to tested attacks, but no cylinder guarantees that forced entry cannot occur.`,
    faqs: [
      ...
      },
      {
        ...
        a: 'For most front doors, I recommend a thumbturn cylinder because it allows you to exit quickly without a key in an emergency such as a fire. The exception is if your door has a letterbox or glass panel near the lock, where someone could reach through and turn the thumbturn. In that case a double cylinder or a letterbox restrictor is a better option.',
      },
      {
  ...
  'bs3621-vs-anti-snap-vs-smart-lock': {
    body: `## Three Approaches to Door Security
...
- **5-lever mechanism** — the key has to lift five internal levers to the correct height before the bolt will move
- **20mm deadbolt throw** — the bolt extends at least 20mm into the frame
- **Anti-pick features** — curtain or false notches to resist picking
- **Anti-drill protection** — hardened steel inserts in the bolt and body
- **BSI Kitemark** — independently tested and certified
...
- Requires a rectangular hole cut into the door body — fitting is more involved
- You need a key to lock from inside (fire safety consideration)
- Can be expensive to fit if cutting a new mortice — see my [five-lever mortice guide](/blog/five-lever-mortice-deadlock-guide) for details
...
- **Sacrificial break point** — designed to snap in a controlled way that keeps the door locked
- **Anti-drill, anti-pick, anti-bump** — multiple layers of attack resistance
- **TS007 rated** — the relevant British Standard for euro cylinders (read my [TS007 vs SS312 guide](/blog/ts007-vs-ss312-lock-standards) for more)
- **Quick to fit** — a direct replacement for your existing cylinder
...
- The cylinder is only part of the picture — the multipoint lock mechanism matters too
...
- **Remote access** — lock/unlock from anywhere via an app (with a WiFi bridge)
- **Access sharing** — send virtual keys to family, guests, or tradespeople
- **Activity logs** — see when the door was locked and unlocked
- **Auto-lock** — some models lock automatically after a set time
...
A Yale nightlatch — often just called "a Yale" — is the lock you can slam shut. It has a spring-loaded latch bolt (the curved bit that clicks into the frame) and sits on the inside surface of the door. The cylinder (where you put the key) pokes through from the outside.
...
- Standard nightlatches can be opened by slipping a credit card or strip of plastic between the door and frame — this is called **loiding**
- **Deadlocking nightlatches** have a secondary bolt that, when the door is fully closed, prevents loiding

### Common Types

- **Yale 85** — basic rim nightlatch, no deadlock function. Not secure enough for a front door.
- **Yale 89** — deadlocking version, much better. The small secondary bolt deadlocks the latch when the door is closed.
- **ERA and Union equivalents** — similar products from other manufacturers.
- **British Standard nightlatches** — BS8621 rated versions that can be unlocked from inside without a key (fire safety feature).

## What Is a Mortice Deadlock?

A mortice deadlock is a lock fitted inside the body of the door. "Mortice" means it sits in a rectangular pocket (mortice) cut into the edge of the door. "Deadlock" means the bolt doesn't spring — it's thrown by the key and stays locked until you turn the key again.
...
- Bolt strength — the bolt resists sawing and forcing
...
A mortice deadlock gives you security — once that bolt is thrown, it takes serious force or the right key to open the door. But you have to remember to lock it with the key every time.
...
Not necessarily. Older BS3621 locks may have been superseded, and some locks carry a BS number for a different standard. Look for the **BSI Kitemark** — the heart-shaped symbol — alongside the BS3621 marking. That confirms independent testing.
...
If you want to discuss smart-lock compatibility, call me on 024 7522 4730 before buying. For independently certified mechanical options, see the [lock upgrade service](/services/lock-upgrade) and [published prices](/prices).`,
    faqs: [
      ...
      },
      {
        ...
        a: 'In theory there are digital vulnerabilities, but in practice residential smart lock hacking is extremely rare. Burglars overwhelmingly use physical methods like lock snapping, kicking doors, or breaking windows. That said, you should keep your smart lock firmware updated, use strong passwords, and enable two-factor authentication on the app.',
      },
      {
...
1. **Open your door** and look at the **faceplate** — the metal strip on the edge of the door where the bolt comes out
...
1. Remove the old lock by unscrewing the faceplate and sliding the lock body out
2. Check the new lock fits the existing mortice pocket — most standard locks are the same case size
3. Insert the new lock, secure the faceplate
4. Mark and drill the keyhole positions on both sides of the door
...
1. Mark the position of the lock on the edge of the door (typically about a third of the way up from the bottom)
2. Drill a series of holes to the correct depth
3. Chisel out the mortice pocket to create a clean rectangular hole
4. Fit the lock body and faceplate
5. Drill keyholes on both sides
6. Cut a recess in the door frame for the bolt (the strike pocket)
7. Fit the strike plate with long screws into the frame timber
8. Fit escutcheons
9. Test extensively
...
If your mortice lock needs attention, call me on 024 7522 4730 with its visible markings and the door symptoms. I will confirm compatible-product availability and whether the work may require another visit. Check the [prices page](/prices) for current rates, or see my [Yale vs mortice deadlock guide](/blog/yale-vs-mortice-deadlock) for the checks that inform a wooden-door specification.`,
    faqs: [
      ...
      },
      {
        ...
        a: 'First try lubricating the keyhole with a graphite powder or PTFE spray — never WD-40. If that does not help, the levers may be worn or corroded, or the bolt may be misaligned with the strike plate. A locksmith can diagnose the issue on-site. Sometimes a service fixes the problem, but if the lock is old or damaged it is better to replace it with a new BS3621 unit.',
      },
    ],
  },

  'ts007-vs-ss312-lock-standards': {
    body: `## TS007 vs SS312 vs Secured by Design: Lock Standards Explained
...
- **Snapping** — attempting to snap the cylinder using tools
- **Bumping** — using a specially cut bump key to jar the pins open
- **Picking** — manipulating the internal mechanism with picks
- **Drilling** — attacking the cylinder with a drill
- **Plug pulling/extraction** — attempting to pull the cylinder core out after a partial snap
...
- **Multiple attack angles** and force levels
...
- **SS312 Diamond certified** (resists snapping)
- **TS007 3-star** (either standalone or with appropriate hardware)
- **From a reputable manufacturer** with proper key registration
